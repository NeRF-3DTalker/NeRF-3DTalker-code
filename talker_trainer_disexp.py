...
class fus_Audio2style(nn.Module):
    def __init__(self, hidden_size=128):
        super(fus_Audio2style, self).__init__()
        '''
        #self.conv_layer_exp = nn.Conv1d(in_channels=1, out_channels=16, kernel_size=3, padding=1)
        #self.pool_layer_exp = nn.MaxPool1d(kernel_size=3, stride=2)
        #self.linear_layer_exp = nn.Linear(48, 29)
        ...
        self.fc2 = nn.Sequential(nn.Linear(128,127), nn.LeakyReLU(0.2, True),nn.Dropout(p=0.5)) # 128
        '''
        self.linear1 = nn.Sequential(nn.Linear(2 * 64, 64), nn.LeakyReLU(0.2, True),nn.Dropout(p=0.5))
        self.linear2 = nn.Sequential(nn.Linear(64, 64), nn.LeakyReLU(0.2, True),nn.Dropout(p=0.5))
              
    def forward(self, f1, f2):
        concat_z = torch.cat([f1, f2], dim=1)
        concat_z=self.linear1(concat_z)
        concat_z=self.linear2(concat_z)
        return concat_z
...
class Trainer(object):
    def __init__(self,config,data_loader):
        ...
        #wav2lip
        #self.wav_model = load_model('/home/lipwav/checkpoints/wav2lip_gan.pth')
        self.Audio2style = Audio2style()
        self.fus_Audio2style = fus_Audio2style()
        ...
        self.optimizer_auds2style=optim.Adam(params=list(self.Audio2style.parameters())+list(self.fus_Audio2style.parameters()), lr=0.0000001, betas=(0.5, 0.999))
        ...
        if self.headnerf_options:
            ...
            #check_dict1 = torch.load('/home/logs/ckpt/epoch_21_ckpt.pth.tar', map_location=torch.device("cpu"))
        ...
        '''
        self.model_q = q_Model().cuda()
        checkpoint_ = torch.load('/fs1/home/tjuvis_2022/CodeTalker/RUN/vocaset/CodeTalker_s1/model/model__.pth.tar', map_location=lambda storage, loc: storage.cpu())
        load_state_dict_(self.model_q, checkpoint_['state_dict'])
        ''' 
    ...
    def train(self):
        ...
        for epoch in range(self.start_epoch,self.epochs):
            ...
            add_file_name+= "_fusemodel"
            
            self.save_checkpoint(
                {'epoch': epoch + 1,
                 'audio2style':self.Audio2style.state_dict(),
                 'fusemodel':self.fus_Audio2style.state_dict(),
                 'fusemodel_optim_state':self.optimizer_auds2style.state_dict(),
    ...
    def train_one_epoch(self, epoch, data_loader, is_train=True):
        ...
        for iter,data_info in loop_bar:
            ...
            with torch.set_grad_enabled(True):
                ...
                fus=self.fus_Audio2style(audiostyle,auds_exp)
                exp_loss=l1_loss(data_info['sad_exp'].squeeze(1).cuda(),fus.cuda())
                '''
            ...
            '''
            self.optimizer_auds2style.zero_grad()
            exp_loss.backward()
            self.optimizer_auds2style.step()
            count=count+1
            exp_loss_total=exp_loss_total+exp_loss
            loop_bar.set_description("Opt, exp_loss: %.6f" % (exp_loss.item()) )  
        ...
        avg_exp_loss=exp_loss_total/count
        print(avg_exp_loss)
        f = open('/home/avg_exp_loss.txt', 'a')
        f.write(str(epoch)+': '+str(avg_exp_loss) + '\n' )    
        f.close()
        #j=0
        #for j in range(79):
    ...
    def load_checkpoint(self, input_file_path='./ckpt/ckpt.pth.tar', is_strict=True):
        ...
        ckpt1 = torch.load('/home/logs/ckpt/epoch_21_ckpt.pth.tar')
