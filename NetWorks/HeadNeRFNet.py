...
class HeadNeRFNet(nn.Module):
    ...
    def calc_color_with_code(self, audiostyle,fg_vps, shape_code, appea_code, FGvp_embedder, 
        ...
        if fine_level:
            FGmlp_FGvp_rgb, FGmlp_FGvp_density = self.fine_fg_CD_predictor(audiostyle,ori_FGvp_embedder, ori_FGvd_embedder)
        else:
            FGmlp_FGvp_rgb, FGmlp_FGvp_density = self.fg_CD_predictor(audiostyle,ori_FGvp_embedder, ori_FGvd_embedder)#neural radiance field torch.Size([1, 256, 1024, 64]),torch.Size([1, 1, 1024, 64])
    ...
    def _forward(
        ...
        #print(appea_code.shape,auds_val.shape)
        ...
        cur_auds_val = audiostyle.unsqueeze(-1).unsqueeze(-1).expand(-1, -1, n_r, self.num_sample_coarse)

        c_ori_res, batch_weight = self.calc_color_with_code(cur_auds_val,
    ...
    def forward(
        ...
        #shape_code=shape_code_i
        #appea_code=appea_code_i
