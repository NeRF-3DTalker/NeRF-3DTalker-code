...
class MLPforNeRF(nn.Module):
    ...
    def _make_layers(self):
        ...
        self.add_module("FeaExt_module_0", nn.Conv2d(self.vp_channels+64, self.h_channel, kernel_size=1, stride= 1, padding=0))
    ...
    def forward(self,audiostyle, batch_embed_vps, batch_embed_vds):
        '''
        batch_embed_vps: [B, C_1, N_r, N_s]
        batch_embed_vds: [B, C_2, N_r, N_s]
        '''
        #print(batch_embed_vps.shape,audiostyle.shape)                

        x = torch.cat([batch_embed_vps, audiostyle], dim = 1)#batch_embed_vps
