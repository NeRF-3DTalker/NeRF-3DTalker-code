...
class VQAutoEncoder(BaseModel):
    ...
    def __init__(self, args):
        ...
        self.quantize = VectorQuantizer(args.n_embed,
                                        args.zquant_dim,
                                        beta=0.25)
        self.quantize_au = VectorQuantizer(args.n_embed,
                                        args.zquant_dim,
                                        beta=0.25)
        self.args = args
    ...
    def decode(self, quant):
        ...
        quant = quant.view(quant.shape[0], -1, self.args.face_quan_num+50, self.args.zquant_dim).contiguous()
        quant = quant.view(quant.shape[0], -1,  (self.args.face_quan_num+50)*self.args.zquant_dim).contiguous()
...
class TransformerDecoder(nn.Module):
  ...
  def __init__(self, args, out_dim, is_audio=False):
    super().__init__()
    self.args = args
    size=128#self.args.hidden_size
    dim=128#self.args.hidden_size
    self.fc = nn.Linear(19584, 16384)
    ...
    self.vertice_map_reverse = nn.Linear(128,128)
    '''
  ...
  def forward(self, inputs):
    ...
    inputs = inputs.contiguous().view(inputs.shape[0],128, 128)
    #print(inputs.shape)
