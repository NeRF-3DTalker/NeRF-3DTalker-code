...
class Dataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        mask_name=str(index)+'_mask'+'.png'
        img_path_ = os.path.join('/home/test/Obama/gen/',img_name_)
        img_path = os.path.join('/home/test/Obama/gt/',img_name)
        mask_path = os.path.join('/home/dataset/Obama/png',mask_name)
        image = cv2.imread(img_path)
        image = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)
        image = image.astype(np.float32)/255.0
        gt_img_size = image.shape[0]
        ...
        input_aus=[]
        with open(os.path.join('/home/dataset/Obama/au',str(index)+".csv")) as file:
            reader = csv.reader(file)
            for  row,index in enumerate(reader):
                if row==1:
                    input_aus.append([float(index[26]),float(index[28]),float(index[31]),float(index[33]),float(index[34])])
