...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if os.path.isfile('/home/RUN/vocaset/CodeTalker_s1/model/model.pth.tar'):
        logger.info("=> loading checkpoint '{}'".format('/home/RUN/vocaset/CodeTalker_s1/model/model.pth.tar'))
        checkpoint = torch.load('/home/RUN/vocaset/CodeTalker_s1/model/model.pth.tar', map_location=lambda storage, loc: storage.cpu())
    ...
    for epoch in range(cfg.start_epoch, cfg.epochs):
        rec_loss_train, quant_loss_train, pp_train = train(train_loader, model, classifier_AU10,classifier_AU14,classifier_AU20,classifier_AU25,classifier_AU26, calc_vq_loss, optimizer,optimizer_classifier, epoch, cfg)
        epoch_log = epoch + 1
        if cfg.StepLR:
            scheduler.step()
        if main_process(cfg):
        ...
        if cfg.evaluate and (epoch_log % cfg.eval_freq == 0):
            rec_loss_val, quant_loss_val, pp_val = validate(val_loader, model,classifier_AU10,classifier_AU14,classifier_AU20,classifier_AU25,classifier_AU26, calc_vq_loss, epoch, cfg)
            if main_process(cfg):
...
import numpy as np
import loss as loss_au


def train(train_loader, model, classifier_AU10,classifier_AU14,classifier_AU20,classifier_AU25,classifier_AU26, loss_fn, optimizer,optimizer_classifier, epoch, cfg):
    ...
    for i, (data,img_tensor_png,mask,aus,data_) in enumerate(train_loader):

        
        current_iter = epoch * len(train_loader) + i + 1
        data_time.update(time.time() - end)
        data_ = data_.cuda()#(cfg.gpu, non_blocking=True)
        ...
        #print(gt_au10.shape0,predict_au10.shape)
        #print(predict_au10.shape,gt_au10.shape,gt_au10[1].unsqueeze(0).shape)
        loss_au_dice10 =loss_au.au_dice_loss(predict_au10, gt_au10)
        loss_au_dice14 =loss_au.au_dice_loss(predict_au14, gt_au14)
        loss_au_dice20 =loss_au.au_dice_loss(predict_au20, gt_au20)
        loss_au_dice25 =loss_au.au_dice_loss(predict_au25, gt_au25)
        loss_au_dice26 =loss_au.au_dice_loss(predict_au26, gt_au26)
        ...
        img_tensor_png = img_tensor_png.cuda()#cuda(cfg.gpu, non_blocking=True)
        img_tensor_png = img_tensor_png.squeeze(0)

        out, quant_loss, info = model(img_tensor_png,AU_fea.unsqueeze(0))
        
        out.permute(1, 2, 0)[mask[0]<0.5]=1.0
        #out[2]=img_tensor_png[0][2]
        #out[1]=img_tensor_png[0][1]                        
        ...
        # LOSS
        #print(data.shape)
        #loss2, _ = loss_fn(out[2], data[0][2], quant_loss, quant_loss_weight=cfg.quant_loss_weight)
        #loss1, _ = loss_fn(out[1], data[0][1], quant_loss, quant_loss_weight=cfg.quant_loss_weight)
        data = data.cuda()#cuda(cfg.gpu, non_blocking=True)
        data = data.squeeze(0)
        loss, loss_details = loss_fn(out, data[0], quant_loss, quant_loss_weight=cfg.quant_loss_weight)
        
        if current_iter%500==0: 
            coarse_fg_rgb_ = (data[0].detach().cpu().permute(1, 2, 0).numpy()* 255).astype(np.uint8)
            res_img=cv2.cvtColor(coarse_fg_rgb_, cv2.COLOR_BGR2RGB)
            cv2.imwrite(os.path.join('/home/test/Obama/','gt_'+str(current_iter) + '.png'),res_img)
        
        total_loss=loss+0.002*whole_AU_loss#+50*loss2+50*loss1
...
def validate(val_loader, model,classifier_AU10,classifier_AU14,classifier_AU20,classifier_AU25,classifier_AU26, loss_fn, epoch, cfg):
    rec_loss_meter = AverageMeter()
    quant_loss_meter = AverageMeter()
    pp_meter = AverageMeter()
    model.eval()
    ...
    with torch.no_grad():
        for i, (data,img_tensor_png,mask,aus,data_) in enumerate(val_loader):
            data_ = data_.cuda()#cuda(cfg.gpu, non_blocking=True)
            #template = template.cuda(cfg.gpu, non_blocking=True)
...
def train(train_loader, model, classifier_AU10,classifier_AU14,classifier_AU20,classifier_AU25,classifier_AU26, loss_fn, optimizer,optimizer_classifier, epoch, cfg):
    ...
    max_iter = cfg.epochs * len(train_loader)
    for i, (data,mask,aus,data_) in enumerate(train_loader):
        ...
        linear2_au25,predict_au25=classifier_AU25(data_)
        linear2_au26,predict_au26=classifier_AU26(data_)
        AU_fea=torch.cat((linear2_au10.repeat(4, 1), linear2_au14.repeat(4, 1),linear2_au20.repeat(4, 1),linear2_au25.repeat(4, 1),linear2_au26.repeat(4, 1)), dim=0)
        #print(linear2_au10.repeat(5, 1).shape)
        #print(gt_au10.shape0,predict_au10.shape)
        #print(predict_au10.shape,gt_au10.shape,gt_au10[1].unsqueeze(0).shape)
        loss_au_dice10 =loss_au.au_dice_loss(predict_au10, gt_au10)
        loss_au_dice14 =loss_au.au_dice_loss(predict_au14, gt_au14)
        ...
        out, quant_loss, info = model(data,AU_fea.unsqueeze(0))
        ...
        if current_iter%500==0: 
            coarse_fg_rgb_ = (out.detach().cpu().permute(1, 2, 0).numpy()* 255).astype(np.uint8)
            res_img=cv2.cvtColor(coarse_fg_rgb_, cv2.COLOR_BGR2RGB)
            cv2.imwrite(os.path.join('/home/test/Macron/',str(current_iter) + '.png'),res_img) 
        # LOSS
        #print(data.shape)
        #loss2, _ = loss_fn(out[2], data[0][2], quant_loss, quant_loss_weight=cfg.quant_loss_weight)
        #loss1, _ = loss_fn(out[1], data[0][1], quant_loss, quant_loss_weight=cfg.quant_loss_weight)
        ...
        if current_iter%500==0: 
            coarse_fg_rgb_ = (data[0].detach().cpu().permute(1, 2, 0).numpy()* 255).astype(np.uint8)
            res_img=cv2.cvtColor(coarse_fg_rgb_, cv2.COLOR_BGR2RGB)
            cv2.imwrite(os.path.join('/home/test/Macron/','gt_'+str(current_iter) + '.png'),res_img) 
...
def validate(val_loader, model,classifier_AU10,classifier_AU14,classifier_AU20,classifier_AU25,classifier_AU26, loss_fn, epoch, cfg):
    ...
    with torch.no_grad():
        for i, (data,mask,aus,data_) in enumerate(val_loader):
            data_ = data_.cuda()#cuda(cfg.gpu, non_blocking=True)
            #template = template.cuda(cfg.gpu, non_blocking=True)
            ...
            linear2_au25,predict_au25=classifier_AU25(data_)
            linear2_au26,predict_au26=classifier_AU26(data_)
            AU_fea=torch.cat((linear2_au10.repeat(52, 1), linear2_au14.repeat(51, 1),linear2_au20.repeat(51, 1),linear2_au25.repeat(51, 1),linear2_au26.repeat(51, 1)), dim=0)
