...
class Audio2Coeff():
    ...
    def __init__(self, sadtalker_path, device):
        ...
        self.optimizer_auds2exp=optim.Adam(params=list(netG.parameters()), lr=0.0000001, betas=(0.5, 0.999))
 
        self.device = device

    def generate(self, batch,exp0, coeff_save_dir, pose_style, ref_pose_coeff_path=None):
    #def generate(self, batch, coeff_save_dir):

        with torch.no_grad():
            #test
            #print(exp0.unsqueeze(1).shape)
            results_dict_exp= self.audio2exp_model.test(batch,exp0.unsqueeze(1))
            return self.optimizer_auds2exp,results_dict_exp
